...
py::object get_range_wrapper(DeviceConfigOption &self) {
    // 明示的にテンプレート引数を指定します
    std::pair<int, int> range = self.get_range<int>();
    return py::make_tuple(range.first, range.second);
}

PYBIND11_MODULE(device_config_python, m) {
    py::class_<DeviceConfigOption>(m, "DeviceConfigOption")
    .def("get_range", &get_range_wrapper);
}
